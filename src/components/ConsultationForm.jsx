...
const ConsultationForm = () => {
 ...
 // Function to generate client ID in "EC-YYMMDD-XXXX" format
const generateClientId = () => {
  const now = new Date();
  const yy = now.getFullYear().toString().slice(-2); // "24" (for 2024)
  const mm = String(now.getMonth() + 1).padStart(2, '0'); // "07" (July)
  const dd = String(now.getDate()).padStart(2, '0'); // "05" (5th day)
  const randomNum = Math.floor(1000 + Math.random() * 9000); // 4-digit random
  return `EC-${yy}${mm}${dd}-${randomNum}`; // Example: "EC-240705-3847"
};

// Auto-set clientId on component mount
useEffect(() => {
  setFormData(prev => ({
    ...prev,
    clientId: generateClientId(), // Auto-generate on load
  }));
  ...
  <div className="form-row">
    ...
    <div className="form-group">
        ...
        <input
          ...
          name="clientId"
          value={formData.clientId}
          onChange={handleChange} // Still needed for form state
          readOnly // Prevents manual edits
          className={`form-input ${errors.clientId ? 'input-error' : ''}`}
          aria-required="true"
        />
      </div>
...
</fieldset> {/* This closing tag was likely missing */}
